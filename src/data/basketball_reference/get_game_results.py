...
class game_results_downloader:
  ...
  def get_data(self, season_year, all_months = True, *args, **kwargs):
      """
      :Purpose: This function will download boxscore .xls spreadsheets from `Basketball-reference <Basketball-reference.com>`_  
                of a given NBA season. After this, it will move each file to the desired directory. This is done 
                by using the web automation provided by the Selenium Python package as well as the os Python 
                package.
      
      :Details: This function can only handle calls for one season.

                When the files are moved, they are moved into a new directory season_year and also renamed in 
                the following way: :code: `{Month}_{season_year}.xls`.
      ...
      :param all_months: Determines whether or not the function will download data for every month of the 
                         specified season.
      ...
      :type kwargs: str
      :param kwargs: Positional arguments that allow the user to specify the months to get data for (if 
                     all_months is set to False) and/or the final path for the spreadsheet. 

                     For specifying months, the keyword arguments themselves (i.e., month_one) is not what 
                     matters thanks to the way you iterate over dictionaries in Python, but what does matter is 
                     the values you set for them; they have to be strings of the months themselves with the 
                     first letter capitalized and the month(s) spelled out entirely and correctly.

                     To specify the final location of the downloaded spreadsheets, the user must use the keyword 
                     'final_path'.If this keyword is not given, the function will default to 
                     ~/data/external/basketball_reference/game_results/season_year/
